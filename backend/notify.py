...
def _build(event: str, order: dict, customer: dict, extra: dict | None):
    extra = extra or {}
    kode = order["kode"]
    nama = customer.get("nama", "Pelanggan")
    if event == "order_created":
        title = "Pengajuan Rental Diterima"
        ...
        html = _email_html(title, f"<p>{escape(body)}</p>", kode)
        return f"Pengajuan Sewa AC Diterima — {kode}", html, wa
    if event == "contract_ready":
